...
int main(int argc, char **argv)
{
    ...
    auto positionnal_init = zed.enablePositionalTracking(ptp);
    if (positionnal_init != sl::ERROR_CODE::SUCCESS)
    {
        std::cerr << "[ZED][ERROR] Can't start tracking of camera" << std::endl;
        return EXIT_FAILURE;
    }
    /// 0.3. Enable camera publishing for fusion:
    ...
    //                          //
    //////////////////////////////
    std::cout << "Start grabing data ... " << std::endl;
...
sl::GNSSData getGNSSData()
{
    ...
    // N.B. For illustrate how to use Geotracking API we generated "fake" GNSS data.
